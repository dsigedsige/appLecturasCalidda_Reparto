...
namespace CaliddaReparto.Web.Controllers
{
    public class Importar_ExcelController : Controller
    {
        ...
        public ActionResult listaCargos_index()
        {
            return View();
        }
        ...
        public string InsertaExcel(HttpPostedFileBase file, int idlocal, string idfechaAsignacion, int idServicio)
        {
            ...
            try
            {
                ...
                if (idServicio == 1)
                {
                    ...
                    //var valor = Objeto_Negocio.Capa_Negocio_TemporalLecturaCargar(fileLocation, ((Sesion)Session["Session_Usuario_Acceso"]).usuario.usu_id, idlocal, idServicio, idfechaAsignacion, NombreArchivo);
                    //if (valor == true)
                    //{
        ...
        [HttpPost]
        public string InsertaExcel_Enel(HttpPostedFileBase file, int idlocal, string idfechaAsignacion, int idServicio, int anio, int mes)
        {
            ...
            try
            {
                ...
                NCorte Objeto_Negocio = new NCorte();
                loDatos = Objeto_Negocio.Capa_Negocio_ListaExcel_enel(fileLocation, ((Sesion)Session["Session_Usuario_Acceso"]).usuario.usu_id, idlocal, idfechaAsignacion, anio, mes);
        ...
        [HttpPost]
        public string get_datosCargos(int anio, int mes, string sector, int tipoCargo)
        {
            object loDatos = null;
            try
            {
                var usuario = ((Sesion)Session["Session_Usuario_Acceso"]).usuario.usu_id;
                NCorte obj_negocio = new NCorte();
                loDatos = obj_negocio.Capa_Negocio_get_datosCargo(anio, mes, sector, tipoCargo,  usuario);
                return _Serialize(loDatos, true);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public string get_datosCargosPDF(string idscargo, int tipoCargo)
        {
            object loDatos = null;
            try
            {
                var usuario = ((Sesion)Session["Session_Usuario_Acceso"]).usuario.usu_id;
                NCorte obj_negocio = new NCorte();
                loDatos = obj_negocio.Capa_Negocio_get_datosCargoPDF(idscargo, tipoCargo, usuario);
                return _Serialize(loDatos, true);
            }
            catch (Exception ex)
            {
