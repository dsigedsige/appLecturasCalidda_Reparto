...
namespace DSIGE.Dato
{
    public class Cls_Dato_AsignaOrdenTrabajo
    {
        ...
        public object Capa_Dato_set_descargarFotosCargos(int id_cargo, int id_usuario)
        {

            cadenaCnx = System.Configuration.ConfigurationManager.ConnectionStrings["dataSige"].ConnectionString;
            string rutaDescarga = ConfigurationManager.AppSettings["Archivos"];
            string[] fileEntries = null;
            ...
            string nombreZip = "fotoCargo_" + id_cargo + ".zip";
            ...
            try
            {
                ...
                {
                    cn.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_S_LISTAR_CARGOS_FOTOS", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@id_cargo", SqlDbType.Int).Value = id_cargo;

                        DataTable dt_detalle = new DataTable();
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                        {
                            da.Fill(dt_detalle);
                            count = dt_detalle.Rows.Count;

                            if (count > 0)
                            {
                                fileEntries = new string[count];
                                foreach (DataRow Fila in dt_detalle.Rows)
                                {
                                    //almacenando un matriz
                                ...
                                ///---quitando las imagenes que no se encuentren en el servidor
                                List<string> listFotos = new List<string>(fileEntries);
                                for (int index = 0; index < listFotos.Count; index++)
                                {
                                    bool nullOrEmpty = string.IsNullOrEmpty(listFotos[index]);
                                ...
                                {
                                    ///--almacenando el archivo zippeado---
                                    pathExist = System.Web.Hosting.HostingEnvironment.MapPath("~/Temp/" + nombreZip);
                                    if (System.IO.File.Exists(pathExist))
                                    {
                                        System.IO.File.Delete(pathExist);
                                    }
                                    ...
                                    res.ok = true;
                                    res.data = rutaDescarga + nombreZip;
                                }
                                else {
                                    res.ok = false;
                                    res.data = "No se encontro las fotos en el servidor";
                                }
                            }
                            else {
                                res.ok = false;
                                res.data = "No se encontro fotos con el cargo seleccionado";
                            }
                        }
