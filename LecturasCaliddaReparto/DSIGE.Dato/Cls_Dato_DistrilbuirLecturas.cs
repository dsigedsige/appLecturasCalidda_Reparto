...
namespace DSIGE.Dato
{
    public class Cls_Dato_DistrilbuirLecturas
    {
           ...
           try
           {
               ...
               using (SqlConnection cn = new SqlConnection(cadenaCnx))
               {
                   ...
                   List<int> list_operarios = new List<int>();
                    ...
                    for (int j = 0; j < ListaTrabajos.Count(); j++)
                    {
                        if (id_operario != Convert.ToInt32(ListaTrabajos[j]))
                        {
                                id_operario = Convert.ToInt32(ListaTrabajos[j]);
                   ...
                   {          
                       using (SqlCommand cmd = new SqlCommand("SP_S_DISTRIBUCION_RELECTURA_CORREO", cn))
                       {
                           ...
                           cmd.Parameters.Add("@fechaAsigna", SqlDbType.VarChar).Value = FechaAsigna;
                           cmd.Parameters.Add("@id_servicio", SqlDbType.Int).Value = servicio;
                           cmd.Parameters.Add("@id_operario", SqlDbType.Int).Value = list_operarios[i];  
                           ...
                           using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                           {
                               ...
                               else
                               {
                                   ...
                                   Res = GenerarArchivoExcel(dt_detalle, list_operarios[i].ToString(), servicio);
                                   ...
                                   if (obj_entidad.email !=null || obj_entidad.email !="")
                                   {
                                    ...
                                    if (string.IsNullOrEmpty(correo_emisor))
                                    {
                                        correo_emisor = dt_detalle.Rows[0]["correo_emisor"].ToString();
                                        contrasenia_emisor = dt_detalle.Rows[0]["contrasenia_emisor"].ToString();
                                    }                                
                                }
                           }
                       }
                   ...
                   {
                        mensaje = SendEmail(listArchivos[i].email, listArchivos[i].ruta, listArchivos[i].nombreFile, correo_emisor, contrasenia_emisor);
                        Thread.Sleep(1000);
                    }
               }       
           }
       ...
       public string SendEmail(string correo, string rutaFile, string nombreFile, string correo_emisor, string contrasenia_emisor)
       {
           string path;
           string mensaje = "";
           try
       ...
       public string GenerarArchivoExcel(DataTable dt_detalles, string id_operario, int servicio)
       {
            ...
            try
            {
                if (servicio == 1)
                {
                    nombreServicio = id_operario+ "_EMAIL_LECTURAS";
                }
                else if (servicio == 2)
                {
                    nombreServicio = id_operario + "_EMAIL_RELECTURAS";
                }
                else if (servicio == 8)
                {
                    nombreServicio = id_operario + "_EMAIL_RECLAMOS";
                }
                ...
                using (Excel.ExcelPackage oEx = new Excel.ExcelPackage(_file))
                {
                    ...
                    if (servicio == 1 || servicio == 2)
                    {
                        ...
                        foreach (DataRow oBj in dt_detalles.Rows)
                        {
                            ...
                            oWs.Cells[_fila, 7].Style.HorizontalAlignment = Style.ExcelHorizontalAlignment.Center; // alinear texto
                    ...
                    else if (servicio == 8) ///reclamos
                    {
                        ...
                        oWs.Cells[1, 7].Value = "Celular";
                        oWs.Cells[1, 8].Value = "TipoCliente";

                        oWs.Cells[1, 9].Value = "Lectura";
                        oWs.Cells[1, 10].Value = "Cod. Observacion";
                        ...
                        foreach (DataRow oBj in dt_detalles.Rows)
                        {
                            ...
                            oWs.Cells[_fila, 6].Value = oBj["distrito"].ToString();
                            oWs.Cells[_fila, 7].Value = oBj["celular"].ToString();
                            oWs.Cells[_fila, 8].Value = oBj["tipoCliente"].ToString();

                            oWs.Cells[_fila, 9].Style.VerticalAlignment = OfficeOpenXml.Style.ExcelVerticalAlignment.Center;
                            oWs.Cells[_fila, 9].Style.HorizontalAlignment = Style.ExcelHorizontalAlignment.Center; // alinear texto
                            oWs.Cells[_fila, 9].Value = "";
                            oWs.Cells[_fila, 10].Value = "";
                ...
                Res = correo + "|" + _ruta + "|" + nombreServicio + _servidor;
            }
            catch (Exception ex)
            {
                throw ex;
