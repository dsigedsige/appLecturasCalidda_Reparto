...
namespace DSIGE.Dato
{
    public class Cls_Dato_Importacion_Lecturas
    {
        ...
        public string Capa_Dato_set_EnviarCorreo_Relectura(int servicio, string fecha_Asigna, int usuario)
        {
            string mensaje = "";
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(cadenaCnx))
                {
                    cn.Open();
                    /// generando los archivos excel
                    using (SqlCommand cmd = new SqlCommand("PROC_S_ENVIAR_CORREO_RELECTURA", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                        {
                            ...
                            else
                            {
                                ...
                                mensaje = GenerarArchivoExcel_Relecturas(dt_detalle, servicio, fecha_Asigna, usuario);
                            }
                        }
                    }
                }
        ...
        public string SendEmail(string correo, string rutaFile, string nombreFile, int servicio, string correo_emisor, string contrasenia_emisor)
        {
            string path;
            string mensaje = "";
            var body = "";
            try
            {
                ////string correos = "cesar.languasco@gmail.com";

                if (servicio == 2)
                {
                    body = "Listado de Re lecturas";
                }
                else if (servicio == 3)
                {
                ...
                using (var smtp = new SmtpClient())
                {
                    ...
                    {
                        UserName = correo_emisor,
                        Password = contrasenia_emisor
                    };
        ...
        public string GenerarArchivoExcel(DataTable dt_detalles, int servicio, string fecha_Asigna, int usuario)
        {
            ...
            try
            {
                ...
                if (servicio ==4)
                {
                  ...
                  correo_emisor = dt_detalles.Rows[0]["correo_emisor"].ToString();
                  contrasenia_emisor = dt_detalles.Rows[0]["contrasenia_emisor"].ToString();
                ...
                {
                    Res = SendEmail(listArchivos[i].email, listArchivos[i].ruta, listArchivos[i].nombreFile, servicio, correo_emisor, contrasenia_emisor);
                }
            }
            catch (Exception ex)
            {
        ...
        public string GenerarArchivoExcel_Relecturas(DataTable dt_detalles, int servicio, string fecha_Asigna, int usuario)
        {
            ...
            try
            {
                ...
                nombreServicio = "EMAIL_RELECTURA_"; 
                ...
                foreach (DataRow row in dt_detalles.Rows)
                {
                    if (id_operario != Convert.ToInt32(row["id_Operario_Lectura"].ToString()))
                    {
                        if (String.IsNullOrEmpty(row["email_operario"].ToString()) == false)
                        {
                            id_operario = Convert.ToInt32(row["id_Operario_Lectura"].ToString());
                            list_operarios.Add(id_operario);
                        }
                    }
                }
                ...
                for (int i = 0; i < list_operarios.Count(); i++)
                {
                    ...
                    using (Excel.ExcelPackage oEx = new Excel.ExcelPackage(_file))
                    {
                        ...
                        foreach (DataRow oBj in dt_detalles.Rows)
                        {
                            id_operario = Convert.ToInt32(oBj["id_Operario_Lectura"].ToString());
                            if (list_operarios[i] == Convert.ToInt32(oBj["id_Operario_Lectura"].ToString()))
                            {
                                ac += 1;
                                oWs.Cells[_fila, 1].Style.VerticalAlignment = OfficeOpenXml.Style.ExcelVerticalAlignment.Center;
                                oWs.Cells[_fila, 1].Style.HorizontalAlignment = Style.ExcelHorizontalAlignment.Center; // alinear texto  
                                ...
                                if (id_operario != Convert.ToInt32(oBj["id_Operario_Lectura"].ToString()))
                                {
                                    id_operario = Convert.ToInt32(oBj["id_Operario_Lectura"].ToString());
                                }
                                _fila++;
        ...
        public string Capa_Dato_save_repartoActualizar(string fechaAsignacion, string fechaMovil, int id_user)
        {
            var Resultado = "";
            ...
            try
            {
                ...
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_I_IMPORTAR_ARCHIVO_REPARTO_ACTUALIZAR", con))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@FechaAsigna", SqlDbType.VarChar).Value = fechaAsignacion;
                        cmd.Parameters.Add("@FechaEjecucion", SqlDbType.VarChar).Value = fechaMovil;
                        cmd.Parameters.Add("@id_usuario", SqlDbType.Int).Value = id_user;
                        cmd.ExecuteNonQuery();
                    }
                }
