    ...
    public class DUbicacion_Lecturas
    {
        ...
        public List<UbicacionOperario> DListaLecturasOperariosGPS(int idOpe, string fechaAsig, string suministro, string medidor, int idEmp,string lista)
        {
            try
            {
                ...
                {
                    using (IDataReader iDr = DatabaseFactory.CreateDatabase().ExecuteReader("SP_S_UBICA_LECTURAS", idOpe, fechaAsig, suministro, medidor, idEmp, Servicios[0], 0, ""))
                    {
                        if (iDr != null)
                        {
                            while (iDr.Read())
        ...
        public List<Ubicacion_Lectura> Capa_Dato_Get_ListaOperariosServicios(int idServicio)
        {
            try
            {
                List<Ubicacion_Lectura> Listlocal = new List<Ubicacion_Lectura>();
        ...
        public List<Ubicacion_Lectura> Capa_Dato_Get_ListaServicio_reporte( int id_usuario)
        {
            try
            {
                List<Ubicacion_Lectura> ListServicio = new List<Ubicacion_Lectura>();
                using (SqlConnection cn = new SqlConnection(cadenaCnx))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_S_SERVICIO_REPARTO", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public List<UbicacionOperario> DSeguimiento_Lectura_II(string pFechaAsiga, int servicio, int operario, string suministro, string medidor)
        {
            ...
            try
            {
                ...
                {
                    using (IDataReader iDr = DatabaseFactory.CreateDatabase().ExecuteReader("SP_S_UBICA_LECTURAS", operario, pFechaAsiga, suministro, medidor, 1, servicio, 0, ""))
                    {
                        if (iDr != null)
                        {
                            while (iDr.Read())
        ...
        public List<UbicacionOperario> DSeguimiento_Lectura_IV(string pFechaAsiga, int servicio, int operario, string suministro, string medidor, string fechaEjecucion, int todosEjecucion)
        {
            List<UbicacionOperario> lOb = new List<UbicacionOperario>();
            try
            {
                ...
                if (servicio > 0) //UN SERVICIO
                {
                    using (IDataReader iDr = DatabaseFactory.CreateDatabase().ExecuteReader("SP_S_UBICA_LECTURAS", operario, pFechaAsiga, suministro, medidor, 1, servicio, todosEjecucion , fechaEjecucion))
                    {
                        if (iDr != null)
                        {
                            while (iDr.Read())
                            {
                                lOb.Add(
                                        new UbicacionOperario()
                                        {
                                            ...
                                            ope_tipo = Convert.ToString(iDr["tipoUsuario"]),
                                            fmovil = Convert.ToString(iDr["fechaLecturaMovil_lectura"]),
                                            VinetaMapa = Convert.ToString(iDr["VinetaMapa"])
                                        }
                                    );
                            }
                        }
